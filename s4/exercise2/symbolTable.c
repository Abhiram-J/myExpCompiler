...
struct symbolNode* createSymbolNode(char* name, int type, int size , int length){
...
void findType(struct symbolTable* gst, struct treeNode* root){
    ...
    if(node != NULL){
        ...
        if(root -> nodetype == idNode){
            if ( node->length == 0 && (root->left == NULL && root->right == NULL) ){
                return;
            }
            if ( node -> length != 0){
                if(root->left != NULL && root->right != NULL && node->size != node->length){
                    return ;
                }
                if(root->left != NULL && root->right == NULL && node->size == node->length){
                    return ;
