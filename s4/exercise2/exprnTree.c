...
int operatorCheck(int nodeType , int lType, int rType){
    ...
    if ( lType == intType && rType == intType ){
        ...
        || nodeType == eqNode || nodeType == neNode || nodeType == modNode || nodeType == idNode ) return 0;
        else return 1;
    }
    return 1;
}
...
reg_index codeGen(struct treeNode *root, FILE *fp){
    ...
    case idNode:
        ...
        if ( root->left != NULL && root->right == NULL){
            ...
            r2 = codeGen(root->left, fp);
            fprintf(fp, "MUL R%d, %d\n", r2, root->symbolTableEntry->length);
            fprintf(fp, "ADD R%d, R%d\n", r1, r2);
            freeReg();
    ...
    case readNode:
                ...
                if ( root->left->left != NULL && root->left->right == NULL){
                    ...
                    r2 = codeGen(root->left->left, fp);
                    fprintf(fp, "MUL R%d, %d\n", r2, root->left->symbolTableEntry->length);
                    fprintf(fp, "ADD R%d, R%d\n", r1, r2);
                    freeReg();
...
void freeGST(struct symbolNode *root){
    if (root == NULL) return;
    freeGST(root->next);
    free(root);
}
