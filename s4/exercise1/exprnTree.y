...
%type <number> TYPE
%token ID NUM PLUS MINUS MUL DIV MOD ASSIGN BEG END READ WRITE ELSE SEMICOLON BREAK CONTINUE DECL ENDDECL INT STR 
...
IDDECL  : ID					{
								$$ = createSymbolNode($<string>1,noType,1 , 1);
							}
		| ID '[' NUM ']'		{
...
expr : expr PLUS expr		{$$ = createTree(noVal, intType, NULL, plusNode, -1,$1,$3);}
     ...
     | expr MOD expr	{$$ = createTree(noVal, intType, NULL, modNode,-1,$1,$3);}
...
IDENTIFIER : ID                     { struct treeNode* temp = createTree(noVal, noType, $<string>1, idNode , 0,NULL,NULL);
            ...
            | ID '[' expr ']' '[' expr ']'  { struct treeNode* temp = createTree(noVal, noType, $<string>1, idNode, 0 ,$3,$6);
                                            findType(gst,temp);
                                            if ( temp->symbolTableEntry->size <= evaluate($3)) yyerror("Array index out of bounds"); // need to check for out of bounds
                                            $$ = temp;}
            ;
