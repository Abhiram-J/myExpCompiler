...
reg_index codeGen(struct treeNode *root, FILE *fp)
{
    ...
    int r1, r2, r3, idPos, label1, label2,offset;
    switch (root->nodetype)
    {
    case numNode:
        r1 = getReg();
    ...
    case tipeNode:
        ...
        if ( root -> localSymbolTableEntry == NULL && root -> globalSymbolTableEntry == NULL) fprintf(fp, "lstentryNULL");
        if ( idPos >= 4096 ){
            fprintf(fp, "MOV R%d, %d\n", r1, idPos);
        }else{
    ...
    case writeNode:
        ...
        else if ( root -> left -> nodetype == tipeNode){
            r1 = codeGen(root->left, fp);
            fprintf(fp, "MOV R%d, [R%d]\n", r1, r1);
            // pushRegisters(fp);
            r2 = getReg();
            printHelper(fp, writeCode, r2, r1);
            freeReg();
        }
        else
        {
    ...
    case funcNode:
        ...
        // if( temp1->arg == NULL) printf("sdfsdf");
        // printf("fsd");
        while (temp1 != NULL)
        {
            r2 = codeGen(temp1->arg, fp);
            if ( temp1 -> arg ->nodetype == tipeNode){
                fprintf(fp,"MOV R%d, [R%d]\n", r2, r2);
            }
            fprintf(fp, "PUSH R%d\n", r2);
            freeReg();
    ...
    case returnNode:
        ...
        if ( root ->left ->nodetype == tipeNode){
             fprintf(fp, "MOV R%d, [R%d]\n", r1, r1);
        }
        fprintf(fp, "MOV [R%d], R%d\n", r2, r1);
        freeReg();
    ...
    case assignNode:
        ...
        // fprintf(fp,"hi");
        if ( root -> left -> nodetype == tipeNode && root -> right -> nodetype == tipeNode){
        // fprintf(fp,"j");
    ...
    case gtNode:
        ...
        r2 = codeGen(root->right, fp);
         if ( root -> right -> nodetype == tipeNode){
            fprintf(fp, "MOV R%d, [R%d]\n", r2, r2);
        }
