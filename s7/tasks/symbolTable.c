...
void assignLocalBindingtoLST(struct localSymbolTable *lst , int offset)
{
    if (lst == NULL){
    ...
    while (temp != NULL){
        temp->binding = currBinding + offset;
        currBinding += 1;
        temp = temp->next;
    }
}
...
void findType(struct globalSymbolTable *gst, struct localSymbolTable *lst, struct treeNode *root){
    ...
    if (lnode != NULL){
        ...
        if ( gnode->cNode != NULL && root->nodetype != funcNode){
...
int checkFuncDeclaration(struct globalSymbolTable *gst, char *name, struct paramNode *paramList , int type, char* tname)
{
    struct globalSymbolNode *temp = gst->head;
    while (temp != NULL){
        if (strcmp(temp->name, name) == 0){
            ...
            if (type == noType && strcmp(tname, temp->typeName) != 0){
                return 1;
            }
            struct paramNode *temp1 = temp->paramList;
            struct paramNode *temp2 = paramList;
            while (temp1 != NULL && temp2 != NULL){
                if (temp1->type != temp2->type ){           // || strcmp(temp1->name, temp2->name) != 
                    return 1;                               // more strict checking
                }
                temp1 = temp1->next;
                temp2 = temp2->next;
            }
